      ...
      <div className="w-full max-w-md rounded-xl border border-[#E5E7EB] bg-white p-6 shadow-lg">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-100 text-emerald-600"><ClipboardCheck size={20} /></span>
            <div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button disabled={!qty || Number(qty) <= 0} onClick={confirm} className="h-10 rounded-xl bg-emerald-600 px-5 text-sm font-medium text-white hover:bg-emerald-500 disabled:opacity-50">Confirmar Apontamento</button>
        </div>
      </div>
    </div>
  );
