...
export default function Topbar() {
  ...
  return (
    <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        <p className="text-sm text-emerald-600">{greet}, {name.split(' ')[0]}</p>
      ...
      <div className="flex items-center gap-2">
        ...
        <Link to="/nova-solicitacao" className="flex h-11 items-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-medium text-white shadow-sm transition hover:bg-emerald-500"><Plus size={17} />Nova Solicitação</Link>
      </div>
    </header>
  );
}